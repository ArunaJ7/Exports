...
def excel_case_distribution_transaction_batch_distribution_array_detail(case_distribution_batch_id, batch_seq):
    ...
    try:   
            ...
            # Check case_distribution_batch_id parameter
            ...
            # Check batch_seq parameter
            if batch_seq is not None:
                if batch_seq == 1:
                    drc_transaction_batch_list_query["batch_seq"] = batch_seq
                elif batch_seq == 2:
                    drc_transaction_batch_list_query["batch_seq"] = batch_seq
               
                else:
                    raise ValueError(f"Invalid case distribution batch id '{batch_seq}'. Must be one of: 1, 2")
...
def create_distribution_table(wb, data, filters=None):
    ...
    try:
        ...
        # Main Header
        ws.merge_cells(start_row=row_idx, start_column=1, end_row=row_idx, end_column=len(DISTRIBUTION_TRANSACTION_BATCH_DISTRIBUTION_HEADERS))
        ...
        # Data Table Headers
        header_row = row_idx
        for col_idx, header in enumerate(DISTRIBUTION_TRANSACTION_BATCH_DISTRIBUTION_HEADERS, 1):
        ...
        last_col_letter = get_column_letter(len(DISTRIBUTION_TRANSACTION_BATCH_DISTRIBUTION_HEADERS))
        ...
        for col_idx in range(1, len(DISTRIBUTION_TRANSACTION_BATCH_DISTRIBUTION_HEADERS) + 1):
