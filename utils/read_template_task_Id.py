...
class Get_Template_Task_ID_Singleton:
    _instance = None
    ...
    def __new__(cls):
        if cls._instance is None:
            with cls._lock:
                if cls._instance is None:
                    cls._instance = super(Get_Template_Task_ID_Singleton, cls).__new__(cls)
    ...
    def get_template_task_id(self):
        ...
        try:
            ...
            task_id = self.config[section].getint('TEMPLATE_TASK_ID')
            return task_id
        except Exception as e:
            print(f"Error getting TEMPLATE_TASK_ID: {e}")
            return None
